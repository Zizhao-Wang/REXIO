...
DEFINE_double(read_write_ratio, 50, "read write ratio (50 means 50%)");
DEFINE_bool(enable_stats,        false,    "Print histogram of operation timings");
DEFINE_bool(read_test_start,        false,    "Print histogram of operation timings");
...
void bench_testing(void)
{
     ...
     Trace * read_trace = new TraceZipfian(1000 + 1* 345,1,FLAGS_num);
     ...
     uint64_t read_write_ratio = FLAGS_read_write_ratio; 

     for(uint64_t i=1;i<=FLAGS_num;i++)
     {
          memset(key_buffer, 0, KEY_SIZE);
          memset(value_buffer, 0, value_size);
          uint64_t operation_start_time =0;
          uint64_t operation_end_time = 0;
          if(i%100 <= read_write_ratio){
               ...
               operation_start_time = clock();
               InsertNode(key_buffer, value_buffer);
               operation_end_time = clock();

          }else{
               data_point = is_random?(read_trace->Next()%FLAGS_range):i;
               for (size_t j = 0; j < sizeof(uint64_t) && j < KEY_SIZE; ++j) 
               {
                    key_buffer[KEY_SIZE - error_bound - 1 - j] = static_cast<char>((data_point >> (8 * j)) & 0xFF);
               }
          ...
          if(FLAGS_enable_stats && (i % FLAGS_stats_interval == 0) )
          {
               endTime = clock();
     ...
     if(FLAGS_read_test_start)
     {
          ...
          for(uint64_t i=1;i<=FLAGS_num;i++)
          {
               ...
               if (result == data_point) {
                    ...
                    printf("Not Found: key:%lu value:%lu\n", i, result);
                    exit(0);
                    not_found_count++; 
               }
               total_attempts++; 
